<header x-data="{ mobileMenuOpen: false }" class="flex justify-between bg-slate-800 shadow-md text-white item
">
    <div>
        <a href="{{ route('home') }}" class="block py-navbar-item pl-5"> <span
                class="text-3xl text-orange-600 font-bold">L</span><span class="text-orange-400 text-2xl"><i>et's go
                    Shopping!</></span>
        </a>
    </div>
        ...
        <ul>
            <li>
                <a href="/src/cart.html"
                    ...
                    <!-- Cart Items Counter -->
                    @if (!Auth::guest())
                        <small class="py-[2px] px-[8px] rounded-full bg-red-500">
                            @php
                                $count = DB::table('cart_items')->count();
                            @endphp
                            {{ $count }}
                        </small>
                    @endif
            ...
            @if (!Auth::guest())
                <li x-data="{ open: false }" class="relative">
                    ...
                    <ul x-show="open" x-transition class="z-10 right-0 bg-slate-800 py-2">
                        <li>
                            <a href="{{ route('profile.home') }}" class="flex px-3 py-2 hover:bg-slate-900">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        ...
                        <li>
                            <a href="/src/watchlist.html" class="flex items-center px-3 py-2 hover:bg-slate-900">
                                ...
                                <small x-transition class="py-[2px] px-[8px] rounded-full bg-red-500"></small>
                            </a>
                        </li>
                        ...
                        <li class="hover:bg-slate-900">
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit" class="flex px-3 py-2 hover:bg-slate-900 w-full">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <ul class="grid grid-flow-col items-center">
            <li>
                <a href="{{ route('cart') }}"
                    ...
                    @if (!Auth::guest())
                        <small class="absolute z-[100] top-4 -right-3 py-[2px] px-[8px] rounded-full bg-red-500">
                            @php
                                $count = DB::table('cart_items')->count();
                            @endphp
                            {{ $count }}
                    ...
                    <ul @click.outside="open = false" x-show="open" x-transition x-cloak
                        ...
                        <li>
                            <a href="/src/watchlist.html"
                                ...
                                <small class="py-[2px] px-[8px] rounded-full bg-red-500"></small>
                            </a>
                        </li>
                        <li>
